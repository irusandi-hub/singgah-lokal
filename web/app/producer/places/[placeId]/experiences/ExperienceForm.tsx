...
function emptyExperienceForm() {
  return {
    id: "", title: "", shortDescription: "",
    description: "", durationMinutes: "60",
    capacity: "", minPartySize: "1",
    maxPartySize: "6", ageRequirement: "",
    prerequisites: "", meetingPoint: "",
    highlights: "", dayOfWeek: "Monday",
    startTime: "09:00", endTime: "15:00",
    scheduleStatus: "requires_confirmation",
  };
}

export default function ExperienceForm({ placeId, placeTimezone, experience, onSaved }: Props) {
  // NEW vs EDIT is explicit: `experience` present = edit an existing record
  // (saved values are the initial state); absent = NEW entry, always empty.
  // The key marker forces re-initialization on NEW↔EDIT remounts so stale
  // transient input from a previous mount cannot resurrect.
  const isEdit = Boolean(experience);
  const [form, setForm] = useState(
    experience
      ? {
          id: experience.id, title: experience.title, shortDescription: experience.shortDescription,
          description: experience.description, durationMinutes: String(experience.durationMinutes),
          capacity: experience.capacity?.toString() ?? "", minPartySize: String(experience.minPartySize),
          maxPartySize: String(experience.maxPartySize), ageRequirement: experience.ageRequirement ?? "",
  ...
  async function submit(event: React.FormEvent) {
    ...
    setMessage(`Tersimpan sebagai ${data.status}`);
    if (!experience) setForm(emptyExperienceForm());
    onSaved?.(data);
  }
