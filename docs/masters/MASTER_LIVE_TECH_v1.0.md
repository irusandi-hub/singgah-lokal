...
Policy §12.1 item 1: provider = Cloudflare Stream; ingest RTMPS/SRT; playback HLS/DASH; recording disabled at provider.
...
- **Boundary rule:** **only server-side code talks to Cloudflare.** Clients never receive stream keys/tokens — they receive only opaque `live_input_id`-derived playback IDs **scoped server-side** (signed playback token, short TTL) per policy §8 "server-issued, scoped means".
